* ServletContainerInitializer：启动容器时负责加载相关配置
    ...
    容器启动时会自动扫描当前服务中 ServletContainerInitializer的实现类，
        并调用其 onStartup方法，其参数Set<Class<?>> c，
        可通过在实现类上声明注解javax.servlet.annotation.HandlesTypes(xxx.class)注解自动注入，
        @HandlesTypes会自动扫描项目中所有的 xxx.class的实现类，并将其全部注入 Set。
...
* SpringServletContainerInitializer
    ...
    从中可以看出，WebApplicationInitializer才是我们需要关心的接口，
        我们只需要将相应的 servlet，filter，listener等硬编码到该接口的实现类中即可。
* 比如：
    * xml配置：
           <!-- Log4jConfigListener -->  
           <context-param>  
    ...
    * 编码配置：
    ...
    Spring为我们提供了一些WebApplicationInitializer的抽象类，我们只需要继承并按需修改即可，比如：
        1）org.springframework.security.web.context
...
* MVC相关配置，比如加载spring配置文件，声明DispatcherServlet等等，参看下面的对比：
* xml配置：
        <context-param>  
