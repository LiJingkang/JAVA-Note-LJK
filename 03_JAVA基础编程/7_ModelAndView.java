...
实例。   
 ...
 　 最简单的 ModelAndView 是持有 View 的名称返回，之后 View 名称被 view resolver，
        也就是实作 org.springframework.web.servlet.View 接口的实例解析，
        例如 InternalResourceView 或 JstlView 等等：     

ModelAndView(String viewName)  
 　如果您要返回Model对象，则可以使用Map来收集这些Model对象，然后设定给ModelAndView，
        使用下面这个版本的ModelAndView：    
            ModelAndView(String viewName, Map model)  
            Map对象中设定好key与value值，之后可以在视图中取出，如果您只是要返回一个Model对象，
            则可以使用下面这个ModelAndView版本：    
...
可以接收Object类型的对象，ModelAndView
 ...
 ModelAndView 也可以接收没有明显名字的对象，原因在于ModelAndView

 将调用spring自己定义的Conventions 类的.getVariableName()方法来为这个model生成一个名字。
 显然，对model而言，名字是必须的。

 Conventions.getVariableName()生成名字的规则是使用对象的类名的小写模式来作model名字。
 当这个model是集合或数组的时候，使用集合的第一个元素的类名加s来作model的名字。
